class OrganizationMailingServicesController < ApplicationController
  ...
  def mailchimp_callback
    ...
    if @service.mailing_service_lists
  ...
  def mailchimp_check
    ...
    if @organization.organization_mailing_services
      ...
      if @organization.organization_mailing_services.first
        ap @organization.organization_mailing_services
        @organization.organization_mailing_services.first.update_or_create_lists
        render json: {mail_chimp: JSON.parse(mailchimp.body) }
      else
        render json: {error: "You have not signed up for mailchimp"}
      end
    end
  end
