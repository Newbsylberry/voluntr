...
angular.module('voluntrApp')
  .controller('NearbyOrganizationsCtrl', function ($scope, $modal, Group, Organization, $stateParams) {
    ...
    Organization.recorded_hours($stateParams.organization_Id, 'nearby_organizations').
    ...
    //
    //var group_set_up = function(group) {
    //  group.rowspan = randomSpan();
    //  group.colspan = randomSpan();
    //  group.bg_color = randomColor();
    //  var arrays = [organization_ids, group.opportunity_ids]
    ...
    //  });
    //  if (result.length > 0) {
    //    $scope.organization_groups.push(group)
    //  } else if (result.length == 0) {
    //    $scope.all_groups.push(group)
    //  }
    //}
